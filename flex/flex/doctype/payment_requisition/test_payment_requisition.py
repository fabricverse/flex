...
class TestPaymentRequisition(FrappeTestCase):
	...
	def _create_project(self):
		all_projects = frappe.get_all("Project", fields=["name"])
		p = frappe.new_doc("Project")
		p.project_name = f"Project {len(all_projects)}"
		p.company = "Malaria Partners Zambia"
	...
	def tearDown(self):
		last_project = frappe.get_last_doc("Project")
		last_project.delete()
		
		# last_cc = frappe.get_last_doc("Cost Center")
		# last_cc.delete()

	def test_project_creation(self):
		company = frappe.get_doc("Company", "Malaria Partners Zambia")
		self.cost_center = frappe.get_last_doc("Cost Center")		
		
		self.assertEqual(f"{self.project_name} - {company.abbr}", self.cost_center.name)
